...
- 모든 토폴로지에 적합합니다.
- 네트워크 크기는 라우터 작업에 영향을주지 않습니다.
- 토폴로지는 트래픽을 다시 라우팅하도록 자동으로 조정됩니다.

> **토폴로지** : 컴퓨터 네트워크의 요소(링크, 노드)를 물리적으로 연결해놓은 방식을 말한다.

 

#### 단점

- 처음에는 구현하기가 복잡 할 수 있습니다.
- 라우팅 업데이트의 브로드 캐스팅 및 멀티 캐스팅은 보안을 약화시킵니다.
- 경로는 현재 토폴로지에 의존합니다.
- CPU, 메모리 및 링크 대역폭과 같은 추가 리소스가 필요합니다.
